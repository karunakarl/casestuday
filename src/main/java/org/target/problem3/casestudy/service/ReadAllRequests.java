		...
		try (FileInputStream fileInputStream = new FileInputStream("F://AssetRequest.xlsx");) {
			...
			while (iterator.hasNext()) {
				...
				RequestModel requestModel = new RequestModel(Double.valueOf(data[0]).longValue(),
						AssetType.valueOf(data[1]), Double.valueOf(data[2]).longValue(), RequestStatus.valueOf(data[3]),
						data[4], row.getRowNum());
	...
	public static List<InventoryModel> getAllInventory() throws IOException {
		...
		try (FileInputStream fileInputStream = new FileInputStream("F://AssetRequest.xlsx");) {
			...
			while (iterator.hasNext()) {
				...
				InventoryModel inventoryModel = new InventoryModel(AssetType.valueOf(data[0]),
						Double.valueOf(data[1]).longValue(), data[2], row.getRowNum());
